...
from openclawV3 import openclaw_sh
from ollama import ollam_sh
import paramiko
...
def run_remote_script1(client, script_text, label):
    ...
    while not url:
        ...
        if "Failed to invoke spank plugin stack" in out0 or "container start failed with error code: 1" in out0:
            print(f"Job {jobID} failed to start. Check the job output for details.")
            input("Press Enter to exit...")
            sys.exit(1)
        time.sleep(1)
...
def run_remote_script2(client, script_text, label):
    ...
    finished = True
    while not finished:
        _, stdout, stderr = client.exec_command(f"cat {jobID}")
        if "FINAL STATUS" in stdout.read().decode("utf-8", errors="replace"):
            finished = True
        elif "Gateway process died" in stdout.read().decode("utf-8", errors="replace"):
            print(f"Job output:\n\n{stdout.read().decode('utf-8', errors='replace')}")
            print(f"Job {jobID} failed to start. Check the job output for details.")
            input("Press Enter to exit...")
            sys.exit(1)
        else:
            print(f"Waiting for {label} to finish launching gateway... {jobID}")
            time.sleep(1)
    return command
...
def run_local_and_open_browser(command, username=None, password=None):
    ...
    SSH_PORT = 22
    OC_HOST = tokens[-1].split('@')[-1].split(':')[0]
    target_port = 22
    ...
    print(f"Running OpenClaw at: 127.0.0.1:{LOCAL_PORT}/chat?token=1234 "
          f"via {SSH_USER}@{OC_HOST}  (Ctrl+C to stop)")
...
def main():
    ...
    try:
        ...
        backendTemplate = environment.from_string(ollam_sh)
        ...
        backendTemplate = environment.from_string(openclaw_sh)
